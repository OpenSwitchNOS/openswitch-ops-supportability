...
def checkDiagDumpFeatureFile(dut01Obj):
    ...
    str_check = 'Diagnostic dump captured for feature'
    vtysh_cmd = 'diag-dump lldp basic diag.txt  '
    tc_desc = vtysh_cmd + ' test '
...
def checkDiagDumpFeatureFilePath(dut01Obj):
    # Variables
    overallBuffer = []
    finalReturnCode = 0
    diag_file = 'diag.txt'
    vtysh_cmd = 'diag-dump lldp basic ' + diag_file
    tc_desc = vtysh_cmd + ' test '
...
def checkDiagDumpFeatureFileSize(dut01Obj):
    # Variables
    overallBuffer = []
    finalReturnCode = 0
...
def checkUnsupportedDaemon(dut01Obj):
    # Variables
    overallBuffer = []
    finalReturnCode = 0
    str_check = 'server returned error'
    vtysh_cmd = 'diag-dump bgp basic'
    tc_desc = vtysh_cmd + ' test '
...
class Test_diag_dump:
    ...
    def test_diag_dump_feature_file_path(self):
        assert(checkDiagDumpFeatureFilePath(dut01Obj))

    def test_diag_dump_feature_file_size(self):
        assert(checkDiagDumpFeatureFileSize(dut01Obj))

    # negative test case

    def test_diag_dump_unsupported_daemon(self):
        assert(checkUnsupportedDaemon(dut01Obj))
