...
convert_to_datetime(char *buf,int buf_size,const char *str)
{
     ...
     struct tm *tmp;
     strl = strlen(str);
     if(strl>=MIN_SIZE)             /*strl must be greater than or equal to MIN_SIZE (6)*/
     {
         ...
         t_t = atoi(basetime);   /*convert basetime string in to integer*/
         tmp = localtime(&t_t);  /*convert in to local time using localtime() API from time.h*/
...
cli_show_events(void)
{
  ...
  SD_JOURNAL_FOREACH(journal_handle)
  {
    ...
    return_value = sd_journal_get_data(journal_handle, "SYSLOG_IDENTIFIER",
     (const void **)&module_name, &module_length);
    if (return_value < 0)
    {
    ...
    /*to get the value from field using get_value() API*/
    ...
    if(tm!=NULL)
      convert_to_datetime(tm_buf,BUF_SIZE,tm);     /*convert real timestamp to unix timestamp */
