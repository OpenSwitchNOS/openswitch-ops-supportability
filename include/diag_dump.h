...
#define DIAG_BASIC_MAX_FEATURE_LEN  20

#define __STRNCMP(X,Y) \
    ( ( X && Y ) && ( 0 == strcmp ( X, Y )) )
...
#define __VALIDATE_FEATURE(X) \
    ( (X) && ( __STRLEN(X)  <  DIAG_BASIC_MAX_FEATURE_LEN  )  && \
     ( __STRLEN(X) > 0 ) )\
...
{ \
    ...
    */ \
    if( \
            ( argc == 3 ) && \
            __STRNCMP(argv[0], DIAG_DUMP_BASIC_CMD) && \
            __STRNCMP(argv[1], DIAG_BASIC) &&  \
