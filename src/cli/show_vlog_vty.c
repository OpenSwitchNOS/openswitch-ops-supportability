 ...
 */
      ...
      switch(request)
      {
         ...
         case 4:  break; //SET REQUEST only for configuration changes by using show vlog feature/daemon to get the changes
...
cli_showvlog_feature(const char *argv0, const char *argv1)
{
   ...
   char *fun_argv = (char*)malloc(LIST_SIZE);
      ...
      const char *level)
{
   ...
   if( strncasecmp(level,"emer",level_len) && strncasecmp(level,"err",level_len)
      ...
      vty_out(vty,"Invalid loglevel %s",VTY_NEWLINE);
      return CMD_WARNING;
   }
...
DEFUN_NOLOCK (cli_config_set_vlog,
      ...
      VLOG_CONFIG_FEATURE
      VLOG_CONFIG_DAEMON
      VLOG_CONFIG_LOGLEVEL
      VLOG_CONFIG_DESTINATION)
{
