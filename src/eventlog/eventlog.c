...
timer_callback()
{
    ...
    while (index < MAX_EVENT_TABLE_SIZE)
    {
        if( (throttled == TRUE) && index < MAX_THROTTLE_TABLE_SIZE)
        {
            ...
            {
                sprintf(msg,"MESSAGE=ops-evt|%d| Throttled %d Messages",
                       throttle_event[index].event_id,
                      -(throttle_event[index].counter)); /*Positive Value only*/
                ret = sd_journal_send(msg,"MESSAGE_ID=%s",MESSAGE_OPS_EVT,NULL);
                if (ret != 0)
                {
...
log_event_throttle(int counter, char *ev_name,...)
{
  ...
  else
  {
      ...
      VLOG_ERR("Messgae Throttled for Event_id %d",ev_table[index].event_id);
      free(message);
      va_end(arg);
      return -1;
  }
