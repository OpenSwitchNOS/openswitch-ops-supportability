    ...
    if(rc) {
      ...
      switch(request)
      {
         case 1:  /*feature result*/
                  ...
                  vty_out(vty,"========================================%s",VTY_NEWLINE);
                  vty_out(vty,"%-17.17s   %-17.17s%s",feature,(cmd_result+POSITION),VTY_NEWLINE);
                  flag = 1;
                  }
                  break;
         ...
         case 2: /*daemon result*/
                  ...
                  vty_out(vty,"======================================%s",VTY_NEWLINE);
                  vty_out(vty,"%-17.17s %-17.17s%s",daemon,(cmd_result+POSITION),VTY_NEWLINE);
                  break;

         case 3: /*show vlog result*/
                  /*flag == 0 means first time displays feature and deamon*/
                  if(flag == 0) {
                  vty_out(vty,"%-15.15s %-13.13s %-18.18s%s",feature,daemon,(cmd_result+POSITION),VTY_NEWLINE);
                  flag = 1;
                  break;
                  }
                  if(flag == 1) {
                     /*flag == 1 means displays next daemons of corresponding feature*/
                  vty_out(vty,"                %-13.13s %-18.18s%s",daemon,(cmd_result+POSITION),VTY_NEWLINE);
                  }
                  break;

         case 4:  break; /*SET REQUEST only for configuration changes by using show vlog feature/daemon to obtain the changes*/
...
cli_showvlog_feature(const char *argv0, const char *argv1)
{
   ...
   if(!initialized) {
      ...
      if(feature_head == NULL){
         vty_out(vty,"Error in retrieving the mapping of feature names to daemon names%s",VTY_NEWLINE);
         FREE(fun_argv);
         return CMD_WARNING;
      }
   }
   ...
   if(!strcmp_with_nullcheck(argv0,DAEMON)){
         ...
         rc = vtysh_vlog_interface_daemon(NULL,(char *)argv1,&fun_argv,fun_argc,request);
      ...
      const char *level)
{
   ...
   if( strncasecmp(destination,"syslog",dest_len) &&
      ...
      /*if destination becomes all then copy "any" to destination before passing to vlog/set*/
   if( !strncasecmp(destination,"all",dest_len)) {
         strncpy((char *)destination,"any",dest_len);
   }

   if( strncasecmp(level,"emer",level_len) && strncasecmp(level,"err",level_len)
       && strncasecmp(level,"warn",level_len) && strncasecmp(level,"dbg",level_len)
          && strncasecmp(level,"info",level_len) && strncasecmp(level,"off",level_len) &&
          strncasecmp(level,"all",level_len)){
      vty_out(vty,"Invalid log level %s",VTY_NEWLINE);
      return CMD_WARNING;
   }
     /*if level becomes all then copy "any" to level before passing to vlog/set*/
   if( !strncasecmp(level,"all",level_len)) {
         strncpy((char *)level,"any",level_len);
   }
   ...
   if(strcmp_with_nullcheck(type,FEATURE) == 0)
   {
      ...
      if(iter != NULL) {
         ...
         iter_daemon = iter->p_daemon;
         while(iter_daemon) {
               /*Feature name is passing as an argument to vtysh_vlog_interface_daemon*/
            rc = vtysh_vlog_interface_daemon(iter->name,iter_daemon->name,fun_argv,
                  fun_argc, request);
            if (!rc) {
               VLOG_DBG("daemon :%s , rc:%d",iter_daemon->name,rc);
...
DEFUN_NOLOCK (cli_config_set_vlog,
      ...
      "vlog (feature|daemon) NAME (SYSLOG | FILE | ALL) (EMER | ERR | WARN | INFO | DBG | ALL | OFF)",
      VLOG_CONFIG
      VLOG_CONFIG_FEATURE
      VLOG_CONFIG_DAEMON
      SHOW_VLOG_NAME
...
cli_showvlog_config()
{
   ...
   for(iter = feature_head ; iter != NULL ; iter = iter->next)
   {
      if(iter) {
         iter_daemon = iter->p_daemon;
         while(iter_daemon) {
            rc = vtysh_vlog_interface_daemon(iter->name,iter_daemon->name,&fun_argv,fun_argc,request);
             if (!rc) {
                VLOG_DBG("daemon :%s , rc:%d",iter_daemon->name,rc);
             }
            iter_daemon = iter_daemon->next;
         ...
         flag = 0;
               ...
               vty_out(vty,"Failed to capture vlog information:%s %s",iter->name, VTY_NEWLINE);
               FREE(fun_argv);
               return CMD_WARNING;
        }
   }
