              ...
              int exact_match)
{
    ...
    OVSREC_SYSLOG_REMOTE_FOR_EACH_SAFE(row,next,idl)
    {
        ...
        /* This function is used primary in two context
         1. To find a similar record, ie., record that is matching just the
            remote address.  Currently we support only one configuration per
            remote, hence we using find similar record when we are replacing
            existing record with new one for the same remote host
         2. To find exact match, ie., record matching all the user provided
            input.  Used by delete configuration feature (no logging )
        */
        /* Compare Host name */
        ...
        )
{
    ...
    row = syslog_remote_get_config(
            remote_host,transport,port_number,severity,exact_match);
              ...
              int is_add)
{
    ...
    if( is_add )
    {
        if(NULL != syslog_remote_get_config(
            remote_host,transport,port_number,severity,1))
        {
    ...
    */
    ...
    if(is_add)
    {
        ...
        {
            vty_out(vty, "Limit of 4 remote syslog servers already reached%s",
                     VTY_NEWLINE);
            cli_do_config_abort(txn);
            return CMD_WARNING;
        }
