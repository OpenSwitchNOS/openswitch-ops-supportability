...
exec_showtech_cmd(const char* cmd)
{
   ...
   if(strncmp_with_nullcheck(BASH_CAT_CMD,trim_cmd,sizeof(BASH_CAT_CMD)) == 0)
   {
      trim_cmd += sizeof(BASH_CAT_CMD);
...
cli_show_tech(const char* feature,const char* sub_feature)
{
   ...
   time(&showtech_end);
   showtech_exec_time = difftime(showtech_start,showtech_end);
   /* Display only if show tech took more than 5 seconds */
   if(showtech_exec_time > 5)
   {
