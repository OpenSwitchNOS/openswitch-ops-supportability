...
add_feature_name (struct feature* element, const char* feature_name)
{
  ...
  element->name = strdup (feature_name);
  return 0;
}
...
add_subfeature_name (struct sub_feature* element, const char* sub_feature_name)
{
  ...
  element->name = strdup (sub_feature_name);
  return 0;
}
...
add_subfeature_desc (struct sub_feature* element, const char* sub_feature_desc)
{
  ...
  element->desc = strdup (sub_feature_desc);
  return 0;
}
  ...
  const char* showtechall)
{
  ...
  if ((!strcmp ("yes", showtechall)) || (!strcmp ("true", showtechall)))
  {
  ...
  const char* showtechfeature)
{
  ...
  if ((!strcmp ("yes", showtechfeature)) || (!strcmp ("true", showtechfeature)))
  {
...
add_ovstable_name (struct ovstable* element, const char* tablename)
{
  ...
  element->tablename = strdup (tablename);
    ...
    if (event.type == YAML_SCALAR_EVENT)
    {
      ...
      switch (event_value)
      {
        case VALUE:
        {
          switch (current_state)
          {
            ...
            case FEATURE_DESC:
            {
              ...
              {
                VLOG_ERR (
                  "Parsing Error while adding New Feature Desc : %s",
                  (const char*) event.data.scalar.value);
                free_show_tech_config ();
                goto cleanup;
              }
            ...
            case SUB_FEATURE_DESC:
            {
              ...
              {
                VLOG_ERR (
                  "Parsing Error while adding New Sub Feature Desc :  %s",
                  (const char*) event.data.scalar.value);
                free_show_tech_config ();
                goto cleanup;
              }
        ...
        case SUB_FEATURE:
        {
          ...
          if (NULL == curr_subfeature)
          {
            VLOG_ERR ("Parsing Error while adding new sub Feature");
            free_show_tech_config ();
            goto cleanup;
          }
